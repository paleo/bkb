  ...
  export class FirstRouter {
    ...
    public addSelectors(selList: string[], urlController: UrlController): Function {
      ...
      for (var i = 0, len = selList.length; i < len; ++i) {
        keys = [];
        regex = FirstRouter.pathToRegexp(selList[i], keys, true, true);
    ...
    public start(opt = {}): void {
      ...
      if (!this.firstRelUrl)
        this.firstRelUrl = FirstRouter.getDefaultFirstRelUrl(this.baseUrl, this.withHashBang);
      if (this.withHistory) {
        window.onpopstate = (e) => {
          try {
    ...
    private doGoTo(relUrl: string, changeHist: boolean): boolean {
      ...
      for (var k in this.selList) {
        ...
        args = FirstRouter.matchRelUrl(relUrl, selProp['regex'], selProp['keys']);
        if (args) {
          up = {
            'relUrl': relUrl,
            'args': args,
