...
function rmRecursive(path, includingThis: boolean, cb: (err, stats) => void) { // TODO rewrite with promises
    ...
    fs.readdir(path, (err, files) => {
      ...
      if (!checkStatus()) {
        for (var i = 0; i < fLength; i++) {
          ...
          rmRecursive(path + '/' + files[i], true, function rmRecursiveCB(err) {
            if (!err) {
              fDeleteIndex++;
              checkStatus();
            } else
...
var ext = {
  ...
  rmRecursive: function (path, includingThis: boolean): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      rmRecursive(path, includingThis, (err) => {
        if (err)
          reject(err);
        else
          resolve();
...
var fsp = {
  ...
  unlink: function (fileName: string): Promise<void> {
    return new Promise<void>((resolve, reject) => {
